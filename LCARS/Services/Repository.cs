...
namespace LCARS.Services
{
    public class Repository : IRepository
    {
        ...
        public BuildStatus GetBuildStatus(string fileName)
        {
            XDocument doc = XDocument.Load(fileName);

            return new BuildStatus
            {
                IsStatic = doc.Root.Attribute("IsStatic").Value == "1",
                Builds = doc.Root?.Elements("Build").Select(buildData => new Build
                {
